...
def home(request):
    return render(request, 'devilx_home/home.html')
    ...
    if request.method == 'POST':
        ...
        if password1 == password2:
            if User.objects.filter(username=username).exists():
                ...
                return render(request, 'devilx_home/register.html')
...
def login(request):
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            return redirect('devilx-home')

        messages.error(request, "invalid username/password")
        return redirect('login')

    return render(request, 'devilx_home/login.html')
...
def password(request):
    ...
    return render(request, 'devilx_home/password.html', {'password': thepassword})
...
def products(request):
    ...
    if request.method == 'POST':
        ...
        all_products = Products.objects.filter(
            (Q(p_category__in=active_categories)) & (Q(p_price__lte=max_price)))

        # print(f"category: {all_products[0].p_image}")
        for i in range(len(all_products)):
            all_products[i].p_image = str(all_products[i].p_image)[
                len("/static/")-1:]
            all_products[i].p_name = (str(all_products[i].p_name)[:40] + '..') if len(
                str(all_products[i].p_name)) > 40 else str(all_products[i].p_name)
        context = {
            'products': all_products,
            'categories': p_categories
        }

        return render(request, 'devilx_home/products.html', context)

    query = request.GET.get('product', '')

    if query != '':
        all_products = Products.objects.filter(p_category__icontains=query).union(Products.objects.filter(
            p_name__icontains=query)).union(Products.objects.filter(p_description__icontains=query))

    for i in range(len(all_products)):
        all_products[i].p_buycount = len(
            Orders.objects.filter(product=all_products[i]))
        all_products[i].save()
        all_products[i].p_image = str(all_products[i].p_image)[
            len("/static/")-1:]
        all_products[i].p_name = (str(all_products[i].p_name)[:40] + '..') if len(
...
def productdetails(request, pk):
    ...
    return render(request, 'devilx_home/productdetails.html', context)
...
def cart(request):
    ...
    if len(product_ids) == 0:
        return render(request, 'devilx_home/cart.html', context={})

    all_products = []
    for prod in product_ids:
        all_products.append(Products.objects.get(id=prod.product.id))

    return render(request, 'devilx_home/cart.html', context={'products': all_products, 'n_items': len(all_products)})
...
def checkout(request, pk):
    ...
    return render(request, 'devilx_home/checkout.html', context=context)
...
def wallet(request):
    return render(request, 'devilx_home/wallet.html', context={'wallet':  Wallet.objects.filter(user=request.user)[0].wallet})


def success(request):
    return render(request, 'devilx_home/success.html')
...
def orders(request):
    ...
    if len(all_orders) == 0:
        context = {}
    return render(request, 'devilx_home/orders.html', context)
...
def order_details(request, pk):
    ...
    if request.method == 'POST':
        ...
        if len(Rating.objects.filter(order=order)) == 0:
            rating_obj = Rating(order=order, rating=rating,
            ...
            f = False
        return render(request, 'devilx_home/order_details.html', context={'order': order, 'f': f, 'rating': rating})

    return render(request, 'devilx_home/order_details.html', context={'order': order, 'f': f, 'rating': rate_value})


def wishlist(request):
    product_ids = Wishlist.objects.filter(user=request.user.id)
    if len(product_ids) == 0:
        return render(request, 'devilx_home/wishlist.html', context={})

    all_products = []
    for prod in product_ids:
        all_products.append(Products.objects.get(id=prod.product.id))

    return render(request, 'devilx_home/wishlist.html', context={'products': all_products, 'n_items': len(all_products)})
